...
export function readProjectIconSource(
  ...
  const request = api.projects
    .readIcon({ projectId })
    .then((result) => {
      if (caches.get(api) !== current) return null;
...
/** Per-connection cache controls; never clears artwork belonging to another node. */
export function projectIconCacheBytes(api: EnvironmentApi): number {
  return caches.get(api)?.resolved.retainedBytes ?? 0;
}
export function clearProjectIconCache(api: EnvironmentApi): void {
  caches.get(api)?.resolved.clear();
  caches.delete(api);
}
