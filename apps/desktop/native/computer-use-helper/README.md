# Ryco computer-use helper

This crate is Ryco's bundled native host for background desktop inspection and control on Windows, macOS, and Linux. It communicates only through newline-delimited JSON on stdin/stdout; diagnostics go to stderr.
...
```bash
ryco-computer-use --state-dir /path/to/private/app-state
```

Send one request per line:

```json
{ "id": 1, "action": "hello", "input": { "protocolVersion": 1, "clientVersion": "ryco" } }
```

The response is either `{ "id", "ok": true, "result" }` or `{ "id", "ok": false, "error", "code" }`. Responses may be out of order. `cancel` cooperatively cancels an active request; `shutdown` closes the host. Run `ryco-computer-use --hello` for a one-shot capability/permission handshake or `--version` for the helper version.
...
From the repository root, `node apps/desktop/scripts/build-computer-use-helper.mjs` builds and stages the current host binary. Release packaging builds Windows x64/arm64, Linux x64-musl, or a macOS x64+arm64 universal binary and validates its `--hello` response.
